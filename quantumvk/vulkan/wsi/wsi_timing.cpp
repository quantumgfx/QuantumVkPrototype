...
namespace Vulkan
{
	...
	void WSITiming::SetDebugEnable(bool enable)
	{
		options.debug = enable;
	}

	void WSITiming::SetLatencyLimiter(LatencyLimiter limiter)
	{
		options.latency_limiter = limiter;
	}

	const WSITimingOptions& WSITiming::GetOptions() const
	{
		return options;
	}

	void WSITiming::SetSwapInterval(unsigned interval)
	{
		...
		uint64_t target = ComputeTargetPresentTimeForSerial(serial_info.serial);
		if (target)
		{
	...
	void WSITiming::UpdateRefreshInterval()
	{
		...
		if (table->vkGetRefreshCycleDurationGOOGLE(device, swapchain, &refresh) == VK_SUCCESS)
		{
			if (!feedback.refresh_interval || options.debug)
				QM_LOG_ERROR("Observed refresh rate: %.6f Hz.\n", 1e9 / refresh.refreshDuration);
			feedback.refresh_interval = refresh.refreshDuration;
		}
		else
			QM_LOG_ERROR("Failed to get refresh cycle duration.\n");
	}

	WSITiming::Timing* WSITiming::FindLatestTimestamp(uint32_t start_serial)
	{
	...
	void WSITiming::UpdatePastPresentationTiming()
	{
				...
				for (uint32_t i = 0; i < presentation_count; i++)
				{
					...
					if (t.wall_serial == feedback.timing_buffer[i].presentID)
					{
							...
							(t.timing.presentMargin > feedback.refresh_interval / 4))
						{
							...
							if (options.debug)
								QM_LOG_INFO("Frame completed very early, but was held back by swap interval!\n");
						}
					}

					UpdateFramePacing(t.wall_serial, t.timing.actualPresentTime, false);
				}
			}
		}

		auto* timing = FindLatestTimestamp(serial_info.serial);
		if (timing && timing->timing.actualPresentTime >= timing->wall_frame_begin)
		{
			...
			if (options.debug)
			{
				QM_LOG_INFO("Have presentation timing for %u frames in the past.\n",
					serial_info.serial - timing->timing.presentID);
			}

			if (int64_t(timing->timing.presentMargin) < 0)
				QM_LOG_ERROR("Present margin is negative (%" PRId64 ") ... ?!\n", timing->timing.presentMargin);

			if (timing->timing.earliestPresentTime > timing->timing.actualPresentTime)
				QM_LOG_ERROR("Earliest present time is > actual present time ... Bug?\n");

			if (timing->timing.actualPresentTime < timing->timing.desiredPresentTime)
			{
				QM_LOG_ERROR("Image was presented before desired present time, bug? (actual: %" PRIu64 ", desired: %" PRIu64 "\n",
			...
			else if (feedback.refresh_interval != 0 && timing->timing.desiredPresentTime != 0)
			{
				...
				{
					QM_LOG_ERROR("*** Image was presented %u frames late "
			...
			if (options.debug)
				QM_LOG_INFO("Total latency: %.3f ms, slack time: %.3f\n", total_latency * 1e-6, timing->slack * 1e-6);
			...
			if (last_frame.serial && timing->wall_serial != last_frame.serial)
			{
					...
					(frame_time_ns > 1.1 * options.swap_interval * feedback.refresh_interval))
				{
					...
					if (platform)
					{
						...
						platform->EventDisplayTimingStutter(serial_info.serial, timing->wall_serial, dropped_frames);
					}
				}

				if (options.debug)
				{
					QM_LOG_INFO("Frame time ID #%u: %.3f ms\n",
						timing->wall_serial,
						1e-6 * frame_time_ns);
				}
			}
	...
	void WSITiming::WaitUntil(int64_t nsecs)
	{
#ifdef __linux__
		timespec ts;
	...
	uint64_t WSITiming::GetWallTime()
	{
#ifndef _WIN32
	...
	void WSITiming::UpdateFramePacing(uint32_t serial, uint64_t present_time, bool wall_time)
	{
		...
		else
		{
			...
			if ((present_time > extrapolated_present_time) || (error < int64_t(feedback.refresh_interval / 2)))
			{
				...
				if (options.debug)
				{
					QM_LOG_INFO("Updating frame pacing base to serial: %u (delta: %.3f ms)\n", pacing.base_serial,
	...
	void WSITiming::UpdateFrameTimeSmoothing(double& frame_time, double& elapsed_time)
	{
		...
		if (delta > std::fabs(target_frame_time * 4.0))
		{
			...
			if (options.debug)
				QM_LOG_INFO("Detected discontinuity in smoothing algorithm!\n");
	...
	uint64_t WSITiming::GetRefreshInterval() const
	{
		return feedback.refresh_interval;
	}

	double WSITiming::GetCurrentLatency() const
	{
		return feedback.latency;
	}

	void WSITiming::LimitLatency(Timing& new_timing)
	{
		if (options.latency_limiter != LatencyLimiter::None)
		{
			...
			if (feedback.refresh_interval)
			{
				int64_t target = int64_t(ComputeTargetPresentTimeForSerial(serial_info.serial));

				if (options.latency_limiter == LatencyLimiter::AdaptiveLowLatency)
				{
					int64_t latency = 0;
					if (GetConservativeLatency(latency))
					{
						// Keep quarter frame as buffer in case this frame is heavier than normal.
						latency += feedback.refresh_interval >> 2;
						WaitUntil(target - latency);

						uint64_t old_time = new_timing.wall_frame_begin;
						new_timing.wall_frame_begin = GetWallTime();
						if (options.debug)
						{
							QM_LOG_INFO("Slept for %.3f ms for latency tuning.\n",
	...
	void WSITiming::BeginFrame(double& frame_time, double& elapsed_time)
	{
		PromoteOrDemoteFrameRate();
		...
		if (options.debug)
			QM_LOG_INFO("Starting WSITiming frame serial: %u\n", serial_info.serial);
		...
		UpdatePastPresentationTiming();
		// Absolute minimum case, just get some initial data before we have some real estimates.
		UpdateFramePacing(serial_info.serial, new_timing.wall_frame_begin, true);
		UpdateFrameTimeSmoothing(frame_time, elapsed_time);
		LimitLatency(new_timing);

		new_timing.wall_frame_target = ComputeTargetPresentTimeForSerial(serial_info.serial);
	}

	bool WSITiming::GetConservativeLatency(int64_t& latency) const
	{
		latency = 0;
	...
	uint64_t WSITiming::ComputeTargetPresentTimeForSerial(uint32_t serial)
	{
	...
	void WSITiming::PromoteOrDemoteFrameRate()
	{
		...
		if (early_frames == total && options.swap_interval > 1)
		{
			...
			SetSwapInterval(options.swap_interval - 1);
			QM_LOG_INFO("Adjusted swap interval down to %u!\n", options.swap_interval);
		}
		else if (frame_drops >= 3)
		{
			...
			{
				SetSwapInterval(options.swap_interval + 1);
				QM_LOG_INFO("Too much hitching detected, increasing swap interval to %u!\n", options.swap_interval);
			}
			else
				QM_LOG_INFO("Still detecting hitching, but reached swap interval limit.\n");
		}
	}

	bool WSITiming::FillPresentInfoTiming(VkPresentTimeGOOGLE& time)
	{
		time.presentID = serial_info.serial;

		time.desiredPresentTime = ComputeTargetPresentTimeForSerial(serial_info.serial);
